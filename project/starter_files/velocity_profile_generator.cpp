...
*/
/*
このクラスは、開始速度から目的の速度までの速度軌道を計算します。
車両が存在できる状態ごとに速度プロファイルを作成する必要があるため、Behavioral Plannnerと連携して機能します。
「Follow_lane」状態では、速度目標を維持するために速度を上げるか下げる必要があります。 
「decel_to_stop」状態では、停止線までスムーズに減速できるプロファイルを作成する必要があります。
これらのケースを処理するための優先順位は、一時停止の標識の処理とその後の名目上の車線の保守です。
実際の速度プランナーでは、これらの状態間の結合を処理する必要がありますが、
簡単にするために、このプロジェクトは各ケースを分離することで実装できます。
すべての軌道について、必要な加速度は_a_max（調整可能な加速度）で与えられます。
ビヘイビアプランナーが参加できる操作/状態のタイプの詳細については、structs.hを参照してください。
*/
...
double VelocityProfileGenerator::calc_distance(const double& v_i,
  ...
  if (std::abs(a) < DBL_EPSILON) {
    ...
    // #001 TODO-距離の計算：一般的な直線加速運動方程式の1つを使用して、
    // 一定の加速/減速「a」でv_i（初速度）からv_f（最終速度）までの移動距離を計算します。
    // ヒントこの関数の説明を見てください。 divを0で処理するようにしてください
    d = (v_f * v_f - v_i * v_i) / (2 * a);  // <- Update #001
  }
  return d;
}
...
double VelocityProfileGenerator::calc_final_speed(const double& v_i,
  ...
  // infinity
  // #002 TODO-最終速度の計算：最終距離を計算します。 ヒント：この関数の説明を見てください。
  // 負の判別式を処理し、その場合はv_f = 0にするようにしてください。
  // 判別式がinfまたはnanの場合、無限大を返します

  double disc = 2 * a * d + v_i * v_i;  // <- Fix this #002
  if (disc <= 0.0) {
    v_f = 0.0;
