...
double collision_circles_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  for (auto wp : spiral) {
    ...
    for (size_t c = 0; c < n_circles && !collision; ++c) {
      ...
      // #001 TODO-円の配置：円はどこにあるべきですか？ 以下のコードは完全ではありません。
      // ヒント：CIRCLE_OFFSETS [c]、正弦および余弦を使用してxとyを計算します：
      // cur_y + CIRCLE_OFFSETS [c] * std :: sin / cos（cur_yaw）
      auto circle_center_x = cur_x + CIRCLE_OFFSETS[c] * std::cos(cur_yaw);  // <- Update #001
      auto circle_center_y = cur_y + CIRCLE_OFFSETS[c] * std::sin(cur_yaw);  // <- Update #001
...
double close_to_main_goal_cost_spiral(const std::vector<PathPoint>& spiral,
  ...
  // #003 TODO-スパイラルの最後のポイントとメインゴールの間の距離：
  // スパイラルの最後のポイント（spiral [n-1]）とメインゴール（main_goal.location）の間の距離を
  // どのように計算しますか。 スパイラル[n-1] .x、スパイラル[n-1] .y、スパイラル[n-1] .zを使用します。
  // main_goal.location.x、main_goal.location.y、およびmain_goal.location.zを使用します
  // 例：main_goal.location.x --spiral [n-1] .x
  auto delta_x = main_goal.location.x - spiral[n - 1].x;  // <- Update #003
