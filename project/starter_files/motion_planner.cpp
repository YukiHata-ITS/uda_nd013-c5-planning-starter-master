...
std::vector<State> MotionPlanner::generate_offset_goals(
  ...
  // #001 TODO-垂直方向：ゴールヨーにpi / 2を追加
  auto yaw = goal_state.rotation.yaw + M_PI/2;  // <- Fix This #001
  ...
  for (int i = 0; i < _num_paths; ++i) {
    ...
    // #002 TODO-オフセットゴール位置：
    // 「オフセット」（上記で計算）を使用してオフセットゴールのxおよびy位置を計算し、
    // ゴールがメインゴールの方向（ヨー）に垂直な線上にあることを確認します。
    // 上記（yaw_plus_90）でこの方向を計算しました。
    goal_offset.location.x += offset * std::cos(yaw);  // <- Fix This #002
    goal_offset.location.y += offset * std::sin(yaw);  // <- Fix This #002
